...
[NOTE]
====
Los componentes de OpenStack exponen una API que es usada por los distintos servicios. Cada componente también suele utilizar una base de datos donde registra sus elementos, estado, operaciones, y demás. La comunicación con la base de datos se realiza con el protocolo https://es.wikipedia.org/wiki/Advanced_Message_Queuing_Protocol[AMQP].
====
...
[TIP]
====
Si dispones de recursos en una plataforma OpenStack que disponga del componente `Heat` instalado, puedes usar https://gist.github.com/ualmtorres/8f1e7720deaa3aefa77678372e37d69e[esta plantilla Heat] para crear la infraestructura. El script denomina `lowcost-net` a la red externa. En la documentación de OpenStack suele denominarse `ext-net`.
====
...
----
<1> Sustituir la línea de `service_plugins` si no se va a instalar el componente _Neutron Load Balancer as a Service (LBaaS)_
<2> Contraseña de RabbitMQ y nombre del nodo de control
